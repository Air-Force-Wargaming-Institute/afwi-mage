...
# Write compiled file to /tmp (writable) instead of read-only /backend
COMPILED_REQ_FILE="/tmp/$SERVICE_NAME-requirements-compiled.txt" 
...
echo "  Using COMPILED_REQ_FILE (in /tmp): $COMPILED_REQ_FILE"
...
if [ -f "$REQ_FILE" ]; then
    ...
    # Call pip-compile directly, writing to $COMPILED_REQ_FILE in /tmp
    pip-compile --allow-unsafe --resolver=backtracking --output-file="$COMPILED_REQ_FILE" "$REQ_FILE" "$EXTRA_REQ_FILE" 2>/tmp/pip_compile_error.log || COMPILE_EXIT_CODE=$?
    if [ $COMPILE_EXIT_CODE -ne 0 ]; then
        ...
        # Call pip-compile directly, writing to $COMPILED_REQ_FILE in /tmp
        pip-compile --allow-unsafe --resolver=backtracking --output-file="$COMPILED_REQ_FILE" "$REQ_FILE" 2>/tmp/pip_compile_error.log || COMPILE_EXIT_CODE=$?
        if [ $COMPILE_EXIT_CODE -ne 0 ]; then
    ...
    # Call pip-compile directly, writing to $COMPILED_REQ_FILE in /tmp
    pip-compile --allow-unsafe --resolver=backtracking --output-file="$COMPILED_REQ_FILE" "$EXTRA_REQ_FILE" 2>/tmp/pip_compile_error.log || COMPILE_EXIT_CODE=$?
    if [ $COMPILE_EXIT_CODE -ne 0 ]; then
...
if [ -f "$COMPILED_REQ_FILE" ] && [ $COMPILE_EXIT_CODE -eq 0 ]; then
    ...
    # Call pip download directly, reading from $COMPILED_REQ_FILE in /tmp
