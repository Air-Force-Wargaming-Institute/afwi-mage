...
def US_Global_Influence_expert(state: GraphState, llm: ChatOpenAI) -> GraphState:
    state_dict = state["keys"]
    question = state_dict["question"]
    whoami = "US_Global_Influence"
    config = load_config()
    if bool(config["SOLO_AGENT"]):
        prompt = PromptTemplate(
            input_variables=["question"],
            template="You are the US Global Influence expert. Here is your job description: You are the Global Influence Expert in a multi-agent system. Track the United States\' efforts to expand its global influence. Your task is to use the moderator guidance and provided documents to answer the question. \n\nYour analysis should \n1. Assess the impact of the US\'s global initiatives on the issue in the query. \n2. Discuss relevant diplomatic efforts, investments, or soft power initiatives. \n3. Analyze how the US\'s actions affect international institutions and norms. \n4. Identify any global trends or reactions that influence the US\'s approach. Use concrete examples from the documents of US\'s global activities and their outcomes.. You have been asked this question: {question}. Please provide a detailed and comprehensive answer to the question, to the best of your abilities."
        )
    ...
    if reflected:
        banner = "\n\n\t---------------------------\n\n\t---US Global Influence AFTER REFLECTION & COLLABORATION---\n\n\t---------------------------\n\n\t"
        print(banner.upper())
        ...
        # print("\tINFO: US Global Influence is using this list of experts while choosing collaborators: \n\t"+expert_agents_str)
...
def US_Global_Influence_collaborator(state: GraphState, llm: ChatOpenAI) -> GraphState:
    banner = "\n\n\t---------------------------\n\n\t---US Global Influence COLLABORATOR---\n\n\t---------------------------\n\n\t"
    print(banner.upper())
    whoami = "US_Global_Influence"
    state_dict = state["keys"]
    question = state_dict["question"]
    ...
    prompt = PromptTemplate(
        ...
        template="You are the US Global Influence expert in a in a collaborative panel of multi-discipline subject matter experts. Here is your job description: You are the Global Influence Expert in a multi-agent system. Track the United States\' efforts to expand its global influence. Your task is to use the moderator guidance and provided documents to answer the question. \n\nYour analysis should \n1. Assess the impact of the US\'s global initiatives on the issue in the query. \n2. Discuss relevant diplomatic efforts, investments, or soft power initiatives. \n3. Analyze how the US\'s actions affect international institutions and norms. \n4. Identify any global trends or reactions that influence the US\'s approach. Use concrete examples from the documents of US\'s global activities and their outcomes.. Your task is to review the report and add to, or rewrite, the areas of the report you were asked to assist with, in order to strengthen the report. You also have access to a set of documents to help you with your task.\n\nReport: {last_actor_analysis}\n\nAreas of the report to improve: {collab_areas}\n\nAnalysis:"
    )
...
def US_Global_Influence_requester(state: GraphState) -> GraphState:
    banner = "\n\n\t---------------------------\n\n\t---US Global Influence REQUESTER---\n\n\t---------------------------\n\n\t"
    print(banner.upper())
    whoami = "US_Global_Influence"
    state_dict = state["keys"]
    question = state_dict["question"]
    ...
    if agent_reflection:
        if shared_state.COLLAB_LOOP:
            ...
            request = f"Here is the feedback from the US Global Influence expert in regards to the analysis generated from the document summery: {agent_reflection}.\n\nPlease consider the expert feedback and retrieve documents that are most related to the question: {question}.\nProvide a summary without embellishment or personal interpertation. Also provide sources or references when possible."

            print(request)

            return {"keys": {**state_dict, "last_actor": whoami, whoami+"_request": request, whoami+"_reflected": True}}
    else:
        request = f"I am the US Global Influence expert in a collaborative panel of multi-discipline subject matter experts. This is my role in the panel: You are the Global Influence Expert in a multi-agent system. Track the United States\' efforts to expand its global influence. Your task is to use the moderator guidance and provided documents to answer the question. \n\nYour analysis should \n1. Assess the impact of the US\'s global initiatives on the issue in the query. \n2. Discuss relevant diplomatic efforts, investments, or soft power initiatives. \n3. Analyze how the US\'s actions affect international institutions and norms. \n4. Identify any global trends or reactions that influence the US\'s approach. Use concrete examples from the documents of US\'s global activities and their outcomes.. Please retrieve documents that are most related to this question: {question}. Please retrieve documents that are most related to the question and provide a summary without embellishment or personal interpertation. Also provide sources or references when possible."
...
# The following will be filled in programmatically when creating a new agent
AGENT_NAME = "US Global Influence"                               #PRC_Domestic_Stability
AGENT_FILE_NAME = "US_Global_Influence"                     #PRC_Domestic_Stability.py
...
COLOR = "#0000FF"                                         #FF0000
CREATED_AT = "2025-02-04T15:39:46.996192"                               #2024-12-15
MODIFIED_AT = "2025-02-07T20:21:35.620117"                             #2025-01-23