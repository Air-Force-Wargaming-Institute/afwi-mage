...
AGENT_NAME = "direct_agent_1"
AGENT_FILE_NAME = "direct_agent_1"
MEMORY_TYPE = "ConversationBufferMemory"
MEMORY_KWARGS = {"max_token_limit": 2000}
AGENT_DESCRIPTION = """AI assistant that does your best to help answer any question or query that the user has."""
AGENT_INSTRUCTIONS = """You are an AI assistant that does your best to help answer any question or query that the user has."""
LLM_MODEL = "gpt-3.5-turbo"
COLOR = "#32CD32"
CREATED_AT = "2025-01-29T19:36:49.401840"
MODIFIED_AT = "2025-01-29T21:12:39.441927"
...
def direct_agent_1_expert(state: GraphState, llm: ChatOpenAI) -> GraphState:
    state_dict = state["keys"]
    question = state_dict["question"]
    whoami = "direct_agent_1"
    config = load_config()
    if bool(config["SOLO_AGENT"]):
        prompt = PromptTemplate(
            input_variables=["question"],
            template="You are the direct_agent_1 expert. Here is your job description: You are an AI assistant that does your best to help answer any question or query that the user has.. You have been asked this question: {question}. Please provide a detailed and comprehensive answer to the question, to the best of your abilities."
        )
    ...
    if reflected:
        banner = "\n\n\t---------------------------\n\n\t---direct_agent_1 AFTER REFLECTION & COLLABORATION---\n\n\t---------------------------\n\n\t"
        print(banner.upper())
        ...
        print("\tINFO: direct_agent_1 is using this list of experts while choosing collaborators: \n\t"+expert_agents_str)
...
def direct_agent_1_collaborator(state: GraphState, llm: ChatOpenAI) -> GraphState:
    banner = "\n\n\t---------------------------\n\n\t---direct_agent_1 COLLABORATOR---\n\n\t---------------------------\n\n\t"
    print(banner.upper())
    whoami = "direct_agent_1"
    state_dict = state["keys"]
    question = state_dict["question"]
    ...
    prompt = PromptTemplate(
        ...
        template="You are the direct_agent_1 expert in a in a collaborative panel of multi-discipline subject matter experts. Here is your job description: You are an AI assistant that does your best to help answer any question or query that the user has.. Your task is to review the report and add to, or rewrite, the areas of the report you were asked to assist with, in order to strengthen the report. You also have access to a set of documents to help you with your task.\n\nReport: {last_actor_analysis}\n\nAreas of the report to improve: {collab_areas}\n\nAnalysis:"
    )
...
def direct_agent_1_requester(state: GraphState) -> GraphState:
    banner = "\n\n\t---------------------------\n\n\t---direct_agent_1 REQUESTER---\n\n\t---------------------------\n\n\t"
    print(banner.upper())
    whoami = "direct_agent_1"
    state_dict = state["keys"]
    question = state_dict["question"]
    ...
    if agent_reflection:
        if shared_state.COLLAB_LOOP:
            ...
            request = f"Here is the feedback from the direct_agent_1 expert in regards to the analysis generated from the document summery: {agent_reflection}.\n\nPlease consider the expert feedback and retrieve documents that are most related to the question: {question}.\nProvide a summary without embellishment or personal interpertation. Also provide sources or references when possible."

            print(request)

            return {"keys": {**state_dict, "last_actor": whoami, whoami+"_request": request, whoami+"_reflected": True}}
    else:
        request = f"I am the direct_agent_1 expert in a collaborative panel of multi-discipline subject matter experts. This is my role in the panel: You are an AI assistant that does your best to help answer any question or query that the user has.. Please retrieve documents that are most related to this question: {question}. Please retrieve documents that are most related to the question and provide a summary without embellishment or personal interpertation. Also provide sources or references when possible."
...
# The following will be filled in programmatically when creating a new agent
AGENT_NAME = "direct_agent_1"                               #PRC_Domestic_Stability
AGENT_FILE_NAME = "direct_agent_1"                     #PRC_Domestic_Stability.py
...
COLOR = "#32CD32"                                         #FF0000
CREATED_AT = "2025-01-29T19:36:49.401840"                               #2024-12-15
MODIFIED_AT = "2025-01-29T21:12:39.441927"                             #2025-01-23