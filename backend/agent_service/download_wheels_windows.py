...
def download_wheels():
    ...
    print("1. Copy the entire agent_service directory to the target machine")
    print("2. Build the Docker image using: docker build -t agent_service .")
    print("3. Run the container using: docker run -p 8001:8001 agent_service")
    
    # Create zip archive option
    create_zip = input("\nWould you like to create a zip archive for transfer? (y/n): ")
    if create_zip.lower() == 'y':
        import shutil
        archive_name = "agent_service_airgapped.zip"
        print(f"Creating {archive_name}...")
        # Get all files and directories in current directory except the archive itself
        shutil.make_archive("agent_service_airgapped", 'zip', ".")
