...
Write-Host "1. Copy the entire agent_service directory to the target machine" -ForegroundColor White
...
Write-Host ""
Write-Host "Would you like to create a zip archive of the agent_service for transfer? (Y/N)" -ForegroundColor Yellow
$response = Read-Host
if ($response -eq "Y" -or $response -eq "y") {
    Write-Host "Creating archive..." -ForegroundColor Yellow
    Compress-Archive -Path ./* -DestinationPath ./agent_service_airgapped.zip -Force
    Write-Host "Created agent_service_airgapped.zip" -ForegroundColor Green
