...
def init_templates():
    ...
    if not list(TEMPLATES_DIR.glob("*.json")):
        ...
        for template_data in TEMPLATES:
            ...
            # Ensure all elements have item_type and parent_uuid is None by default
            if "content" in current_template and "items" in current_template["content"]:
                for item in current_template["content"]["items"]:
                    if item.get("item_type") == "element": # Can also be a section if manually defined in TEMPLATES
                        # Ensure parent_uuid is present, defaulting to None if not already there
                        if "parent_uuid" not in item:
                            item["parent_uuid"] = None 
                    # If it's a section, its elements would be handled by Pydantic schema validation
                    # or if they were defined directly in TEMPLATES like before.
                    # For this simplified approach, we assume sections aren't predefined in TEMPLATES.
