...
Write-Host "1. Copy the entire auth_service directory to the target machine" -ForegroundColor White
Write-Host "2. Build the Docker image using: docker build -t auth_service ." -ForegroundColor White
...
Write-Host ""
Write-Host "Would you like to create a zip archive of the auth_service for transfer? (Y/N)" -ForegroundColor Yellow
$response = Read-Host
if ($response -eq "Y" -or $response -eq "y") {
    Write-Host "Creating archive..." -ForegroundColor Yellow
    Compress-Archive -Path ./* -DestinationPath ./auth_service_airgapped.zip -Force
    Write-Host "Created auth_service_airgapped.zip" -ForegroundColor Green
