...
def download_wheels():
    ...
    print("1. Copy the entire auth_service directory to the target machine")
    print("2. Build the Docker image using: docker build -t auth_service .")
    ...
    if create_zip.lower() == 'y':
        import shutil
        archive_name = "auth_service_airgapped.zip"
        print(f"Creating {archive_name}...")
        # Get all files and directories in current directory except the archive itself
        shutil.make_archive("auth_service_airgapped", 'zip', ".")
