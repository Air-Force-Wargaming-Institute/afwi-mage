...
class LLMManager:
    ...
    def _initialize(self):
        ...
        vllm_model_name = os.getenv('VLLM_MODEL_NAME', config.get('LOCAL_LLM', '/models/DeepHermes-3-Llama-3-8B-Preview-abliterated'))
    ...
    def _fetch_available_models(self, base_url: str) -> List[str]:
        ...
        try:
            ...
            config = load_config()
            default_model = os.getenv('VLLM_MODEL_NAME', config.get('LOCAL_LLM', '/models/DeepHermes-3-Llama-3-8B-Preview-abliterated'))
            if default_model:
