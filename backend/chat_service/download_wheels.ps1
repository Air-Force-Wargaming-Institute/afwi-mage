...
try {
    ...
    Write-Host "`nDownloaded $($wheels.Count) Linux-compatible wheel files to $wheelsDir`:" -ForegroundColor Green
    ...
    Write-Host "3. All Python dependencies are provided as wheel files" -ForegroundColor Yellow
    Write-Host "`nTo use these wheels in an airgapped environment:" -ForegroundColor Green
    Write-Host "1. Copy the entire chat_service directory to the target machine" -ForegroundColor Green
    Write-Host "2. Build the Docker image using: docker build -t chat_service ." -ForegroundColor Green
    Write-Host "3. Run the container using: docker run -p 8009:8009 chat_service" -ForegroundColor Green
    
    # Offer to create a zip file for easy transfer
    Write-Host "`nWould you like to create a zip file of the chat_service directory for transfer? (y/n)" -ForegroundColor Yellow
    $createZip = Read-Host
    
    if ($createZip -eq "y" -or $createZip -eq "Y") {
        $zipPath = Join-Path -Path $PSScriptRoot -ChildPath "chat_service_airgapped.zip"
