...
def regional_dynamics_expert(state: GraphState, llm: ChatOpenAI) -> GraphState:
    """
    The PRC Regional Dynamics Expert
    ...
    if reflected:
        print("\n\n\t---------------------------\n\n\t---REGIONAL DYNAMICS EXPERT AFTER REFLECTION---\n\n\t---------------------------\n\n\t")
        ...
        prompt = PromptTemplate(
            ...
            template="You are the Regional Dynamics Expert in a multi-agent system. Examine the PRC's relationships with neighboring countries and regional powers. Your task is to use the moderator guidance and provided documents to answer the question. Your analysis should: 1. Explain how regional dynamics affect PRC's approach to the issue in the query. 2. Discuss relevant historical context, territorial disputes, or shifting alliances. 3. Analyze the positions and potential reactions of key regional players. 4. Identify any recent regional developments or agreements that impact the issue. Provide specific examples of regional interactions from the documents and their implications.\n\nModerator Guidance: {moderator_guidance}\n\nQuestion: {question}\n\nDocument Summary: {document_summary}\n\nRelevant Documents: {relevant_docs}\n\nAnalysis:"
        )
        ...
        reflection_prompt = PromptTemplate(
            ...
            template="You are a professional analyst who specializes in reviewing and critiquing reports on Chinese regional dynamics, specifically the PRC's relationships with neighboring countries and regional powers. You are tasked to provide a critical analysis on a report about the regional dynamics and provide feedback and identify potential mischaracterizations in the analysis along with calling out information that is not true or that you suspect may be innaccurate. After explaining your findings of the initial report, create a succinct list of instructions for the expert to incorporate as corrections or clarifications in the expert's new draft of their report. The report is as follows: {analysis}.\n\nSupport your points with relevant facts and examples found in the this document summary: {documents_text}.\n\nCritique:"
        )
...
def regional_dynamics_requester(state: GraphState) -> GraphState:
    """
    The PRC Regional Dynamics Requester
    Logically, it is part of the regional_dynamics_expert, but for the sake of functionality, it is separate.
    ...
    It will either request documents that are most relevant to the question, or it will request documents that are most relevant to the feedback from the PRC Regional Dynamics Expert.
