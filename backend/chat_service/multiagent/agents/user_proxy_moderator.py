...
from multiagent.graphState import GraphState
from multiagent.graphState import ExpertState

def user_proxy_moderator(state: GraphState) -> GraphState:
    #Removed
    pass

def get_Moderator_Guidance(state: ExpertState) -> ExpertState:
    llm = LLMManager().llm
    ...
    guidance = chain.invoke({"question": GraphState["Question"],
                                "expert": ExpertState["Expert"],
                                })
    return {f"{ExpertState['Expert']}_moderator_guidance": guidance}