...
def get_Moderator_Guidance(state: ModGuidanceState):
    ...
    whoami = state['expert']
    logger.info(SystemPromptManager().get_prompt("moderator_guidance"))
    llm = LLMManager().get_llm(SystemPromptManager().get_prompt("moderator_guidance")["llm"])  # Uses default model from config
