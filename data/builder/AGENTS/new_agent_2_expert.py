...
AGENT_NAME = "new agent 2"
AGENT_FILE_NAME = "new_agent_2"
MEMORY_TYPE = "ConversationBufferMemory"
MEMORY_KWARGS = {"max_token_limit": 2000}
AGENT_DESCRIPTION = """asdfasdfasdfasdfasdf asdfasdf"""
AGENT_INSTRUCTIONS = """asdfasdfa asdfasdfasdf"""
LLM_MODEL = "gpt-3.5-turbo"
COLOR = "#FF0000"
CREATED_AT = "2025-01-29T21:37:45.178567"
MODIFIED_AT = "2025-01-29T23:11:22.133589"
...
def new_agent_2_expert(state: GraphState, llm: ChatOpenAI) -> GraphState:
    ...
    if bool(config["SOLO_AGENT"]):
        prompt = PromptTemplate(
            ...
            template="You are the new agent 2 expert. Here is your job description: asdfasdfa asdfasdfasdf. You have been asked this question: {question}. Please provide a detailed and comprehensive answer to the question, to the best of your abilities."
        )
    ...
    if reflected:
        banner = "\n\n\t---------------------------\n\n\t---new agent 2 AFTER REFLECTION & COLLABORATION---\n\n\t---------------------------\n\n\t"
        print(banner.upper())
        ...
        print("\tINFO: new agent 2 is using this list of experts while choosing collaborators: \n\t"+expert_agents_str)
...
def new_agent_2_collaborator(state: GraphState, llm: ChatOpenAI) -> GraphState:
    banner = "\n\n\t---------------------------\n\n\t---new agent 2 COLLABORATOR---\n\n\t---------------------------\n\n\t"
    print(banner.upper())
    whoami = "new_agent_2"
    state_dict = state["keys"]
    question = state_dict["question"]
...
def new_agent_2_requester(state: GraphState) -> GraphState:
    banner = "\n\n\t---------------------------\n\n\t---new agent 2 REQUESTER---\n\n\t---------------------------\n\n\t"
    print(banner.upper())
    whoami = "new_agent_2"
    state_dict = state["keys"]
    question = state_dict["question"]
    ...
    if agent_reflection:
        if shared_state.COLLAB_LOOP:
            ...
            request = f"Here is the feedback from the new agent 2 expert in regards to the analysis generated from the document summery: {agent_reflection}.\n\nPlease consider the expert feedback and retrieve documents that are most related to the question: {question}.\nProvide a summary without embellishment or personal interpertation. Also provide sources or references when possible."

            print(request)

            return {"keys": {**state_dict, "last_actor": whoami, whoami+"_request": request, whoami+"_reflected": True}}
    else:
        request = f"I am the new agent 2 expert in a collaborative panel of multi-discipline subject matter experts. This is my role in the panel: asdfasdfa asdfasdfasdf. Please retrieve documents that are most related to this question: {question}. Please retrieve documents that are most related to the question and provide a summary without embellishment or personal interpertation. Also provide sources or references when possible."
...
# The following will be filled in programmatically when creating a new agent
AGENT_NAME = "new agent 2"                               #PRC_Domestic_Stability
AGENT_FILE_NAME = "new_agent_2"                     #PRC_Domestic_Stability.py
...
COLOR = "#FF0000"                                         #FF0000
CREATED_AT = "2025-01-29T21:37:45.178567"                               #2024-12-15
MODIFIED_AT = "2025-01-29T23:11:22.133589"                             #2025-01-23