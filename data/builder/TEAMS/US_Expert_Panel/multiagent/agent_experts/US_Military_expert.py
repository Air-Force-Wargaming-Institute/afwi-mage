...
def US_Military_expert(state: GraphState, llm: ChatOpenAI) -> GraphState:
    ...
    if reflected:
        banner = "\n\n\t---------------------------\n\n\t---US Military AFTER REFLECTION & COLLABORATION---\n\n\t---------------------------\n\n\t"
        print(banner.upper())
...
def US_Military_collaborator(state: GraphState, llm: ChatOpenAI) -> GraphState:
    banner = "\n\n\t---------------------------\n\n\t---US Military COLLABORATOR---\n\n\t---------------------------\n\n\t"
    print(banner.upper())
    whoami = "US_Military"
    state_dict = state["keys"]
    question = state_dict["question"]
    ...
    prompt = PromptTemplate(
        ...
        template="You are the US Military expert in a in a collaborative panel of multi-discipline subject matter experts. Here is your job description: You are the US Military Expert in a multi-agent system. Concentrate on the capabilities, doctrine, and strategic objectives of the United States Armed Forces .\n\nYour task is to use the moderator guidance and provided documents to answer the question. \n\nYour analysis should \n1. Assess potential military actions or responses related to the query. \n2. Provide details on relevant military capabilities, technologies, or strategies. \n3. Explain how military considerations influence the US\'s approach to the issue at hand. \n4. Discuss any recent military developments or exercises relevant to the query. Provide concrete examples and data from the documents to support your analysis.. Your task is to review the report and add to, or rewrite, the areas of the report you were asked to assist with, in order to strengthen the report. You also have access to a set of documents to help you with your task.\n\nReport: {last_actor_analysis}\n\nAreas of the report to improve: {collab_areas}\n\nAnalysis:"
    )
...
def US_Military_requester(state: GraphState) -> GraphState:
    banner = "\n\n\t---------------------------\n\n\t---US Military REQUESTER---\n\n\t---------------------------\n\n\t"
    print(banner.upper())
    whoami = "US_Military"
    state_dict = state["keys"]
    question = state_dict["question"]
    ...
    if agent_reflection:
        if shared_state.COLLAB_LOOP:
            ...
            request = f"Here is the feedback from the US Military expert in regards to the analysis generated from the document summery: {agent_reflection}.\n\nPlease consider the expert feedback and retrieve documents that are most related to the question: {question}.\nProvide a summary without embellishment or personal interpertation. Also provide sources or references when possible."

            print(request)

            return {"keys": {**state_dict, "last_actor": whoami, whoami+"_request": request, whoami+"_reflected": True}}
    else:
        request = f"I am the US Military expert in a collaborative panel of multi-discipline subject matter experts. This is my role in the panel: You are the US Military Expert in a multi-agent system. Concentrate on the capabilities, doctrine, and strategic objectives of the United States Armed Forces .\n\nYour task is to use the moderator guidance and provided documents to answer the question. \n\nYour analysis should \n1. Assess potential military actions or responses related to the query. \n2. Provide details on relevant military capabilities, technologies, or strategies. \n3. Explain how military considerations influence the US\'s approach to the issue at hand. \n4. Discuss any recent military developments or exercises relevant to the query. Provide concrete examples and data from the documents to support your analysis.. Please retrieve documents that are most related to this question: {question}. Please retrieve documents that are most related to the question and provide a summary without embellishment or personal interpertation. Also provide sources or references when possible."
...
# The following will be filled in programmatically when creating a new agent
AGENT_NAME = "US Military"                               #PRC_Domestic_Stability
AGENT_FILE_NAME = "US_Military"                     #PRC_Domestic_Stability.py
...
MODIFIED_AT = "2025-02-07T20:21:40.967410"                             #2025-01-23