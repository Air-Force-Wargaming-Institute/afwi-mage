...
*   **User Story:** As a developer, I need to update the backend data models for reports and templates to support a parent-child hierarchy.
...
*   **User Story:** As a user, I want to visually distinguish Sections from Elements in the config panel.
...
*   **User Story:** As a user, I want to be able to reorder Sections and have their child Elements move with them.
...
*   **User Story:** As a user, I want Sections to always contain at least one Element.
...
*   **User Story:** As a user, I want to manage the relationship between Elements and Sections.
...
*   **User Story (Developer): As a developer, I need to adapt the frontend's underlying structure to support Sections and parent-child relationships, enabling subsequent UI features.**
    *   [X] **State Management (e.g., in `ReportDesignerPage.js` or `ReportConfigPanel.js`):** (Completed in `ReportDesignerPage.js`)
        *   [X] Update the state structure for the report definition to include Sections and their child Elements, including `parent_uuid` for each element.
        *   [X] Ensure functions that modify the report structure (add, delete, reorder) correctly handle the parent-child relationships and `parent_uuid`.
    *   [X] **API Integration:**
        *   [X] Ensure frontend API calls correctly send and receive `parent_uuid` information when saving/loading report definitions.
...
*   **User Story: As a user, I want to visually distinguish Sections from Elements in the config panel so I can easily understand the report structure.**
    *   [X] **UI for Sections (Visual Distinction):**
        *   [X] Create a visual component for "Section" items in `ReportConfigPanel.js`.
        *   [X] Style Sections to be distinct from Elements.
    *   [X] **UI for Elements (Visual Association):**
        *   [X] Adjust Element rendering to show nesting or clear association with a parent Section.

*   **User Story: As a user, I want the report saving functionality to accurately store the new parent-child relationships between Sections and Elements, so my organized report structure is preserved and can be reloaded correctly.**
    *   [X] **Task:** Verify and update the save functionality (likely in `ReportDesignerPage.js` or a related service) to correctly serialize the `items` array (containing both Sections and Elements with `parent_uuid` and `elements` for Sections) when sending data to the backend.
    *   [X] **Task:** Ensure the frontend correctly processes the backend's response after saving, particularly if new IDs or timestamps are generated for sections or elements.
    *   [X] **Task:** Thoroughly test the save and reload process for reports with various configurations:
        *   [X] Reports with multiple sections and nested elements.
        *   [X] Reports with a mix of parented and unparented elements.
        *   [X] Reports where sections might be reordered.
        *   [X] Edge cases, such as saving immediately after section/element creation or deletion.

*   **User Story: As a user, I want to manage Sections and their Elements effectively using intuitive controls.**
    *   [X] **Adding Sections & Elements:**
        *   [X] Implement "Add Section to End" button and its logic (on the left of config panel) // UI and logic complete.
        *   [X] Implement "Insert Section Below" button and its logic (far left, bottom of elements/sections).
        *   [X] Move the "Add Element to End" button to the right of the config panel; update its logic. // Moved to right. Logic currently adds to end of all items, review if this needs to change due to sections.
        *   [X] Move "Insert Element Below" button to the far right within an Element's display area.
        *   [X] Update "Insert Element Below" logic: if the current Element is in a Section, the new Element is added to the same Section.
    *   [X] **Section Collapse/Expand:**
        *   [X] Add expand/collapse controls to Section items.
        *   [X] Implement logic to show/hide child Elements when a Section is expanded/collapsed.
...
*   **User Story: As a user, I want to be able to reorder Sections within the config panel and have their child Elements automatically move with them, maintaining their internal order.**
    *   [X] **Reordering Logic:**
        *   [X] Implement reordering for Sections (e.g., drag-and-drop or up/down buttons).
        *   [X] Implement reordering for Elements within a Section (e.g., up/down buttons).
        *   [X] Ensure child Elements move with their parent Section, maintaining their relative order.

*   **User Story: As a user, I want clear rules for how Sections and Elements interact, ensuring data integrity and ease of use (e.g., Sections always have content, Elements can be moved between Sections or become independent).**
    *   [X] **Element-Section Association Logic:**
        *   [X] Implement logic for the "add to section above" button for unparented Elements.
        *   [X] Implement logic for "remove from parent section" button for parented Elements (element becomes unparented and moves below its former section).
    *   [X] **Section Content & Deletion Rules:**
        *   [X] When a parent Section is deleted, all its child Elements will have their `parent_uuid` set to `null` and become unparented, remaining in the report structure.
        *   [X] Enforce the "Section must have at least one child" rule (by adding a new standard-default element with its `parent_uuid` appropriately set on Section creation or when the last child is deleted).

*   **User Story: As a user, I want to be able to rename sections in the config panel to better organize my report.**
    *   [X] **Section Renaming Logic:**
        *   [X] **Task:** Implement UI for inline editing or a rename button for Section titles in `ReportConfigPanel.js`.
        *   [X] **Task:** Update state management (`handleElementChange` or a new section-specific handler) to reflect section title changes in the section object within the main flat `items` list.
        *   [X] **Task:** Ensure section title changes are persisted correctly when the report is saved (verify backend handles section title updates).

*   **User Story: As a user, I want to delete a section, and its child elements should become unparented and remain in the report, so I don't lose content unintentionally.** (Refines existing deletion logic)
    *   [X] **Section Deletion - Child Handling:**
        *   [X] **Task:** Implement UI for a "Delete Section" button in `ReportConfigPanel.js` (if not already satisfactory).
        *   [X] **Task:** Refine `handleDeleteItem` for sections in `ReportConfigPanel.js`:
            *   [X] Identify all child elements of the deleted section (e.g., from its `elements` array or by filtering the flat list by `parent_uuid`).
            *   [X] For each corresponding child element in the main flat `items` list, set its `parent_uuid` to `null`.
            *   [X] Remove the section object from the main flat `items` list.
            *   [X] Ensure the (now unparented) child elements are correctly positioned in the main flat `items` list (e.g., placed where the section was, maintaining their internal order).
...
*   **User Story: As a user, I want to easily remove an element from its parent section, making it a top-level element, so I can restructure my report.** (Clarification for robust removal)
    *   [X] **Remove Element From Section - Robustness:**
        *   [X] **Task:** Review and debug the "Remove from Section" button logic (`handleRemoveElementFromSection`) in `ReportConfigPanel.js` to ensure:
            *   [X] The element's `parent_uuid` in the main flat `items` list is set to `null`.
            *   [X] The element is removed from its former parent section's `elements` array (if dual state management is maintained).
            *   [X] The element is correctly positioned as a top-level item in the main flat `items` list (e.g., appearing below its former parent section).
            *   [X] If the parent section becomes empty as a result, a new default child element is correctly added to that section's `elements` array AND to the main flat `items` list (with the correct `parent_uuid` linking it to the section).
